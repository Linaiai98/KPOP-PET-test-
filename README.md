...
本插件支持通过Firebase进行跨设备数据同步。

- **无缝同步**: 在多个浏览器或设备间实时同步您的宠物状态。
- **匿名安全**: 无需注册，通过临时的连接码即可安全地同步设备。

#### 如何使用云同步：
1. **在您的主设备上**:
   - 打开扩展设置，找到“☁️ 云同步 (Firebase)”部分。
   - 点击 **“生成连接码”** 按钮。
   - 插件会显示一个6位数的连接码，有效期为10分钟。
2. **在您的新设备上**:
   - 同样打开云同步设置部分。
   - 在输入框中填入您在主设备上获取的6位数连接码。
   - 点击 **“连接”** 按钮。
3. **完成**: 连接成功后，两台设备的数据将自动保持同步。您在一台设备上的任何操作（如喂食、玩耍）都会立即反映在另一台设备上。
4. **断开同步**: 您可以随时在设置中点击 **“断开同步”** 来取消设备的关联。
